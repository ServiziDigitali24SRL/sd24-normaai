    ...
    >
      ...
      >
        <div ref={menuRef} style={{ position: "relative", flexShrink: 0 }}>
          ...
          <input ref={fileInputRef} type="file" accept=".pdf,.doc,.docx,.txt,.xlsx,.csv"
            style={{ display: "none" }}
            onChange={(e) => { const file = e.target.files?.[0]; if (file) onOpenChat?.(context ?? undefined); }}
          />
        ...
        {recording ? (
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, padding: "6px 8px" }}>
            <span style={{ width: 9, height: 9, borderRadius: "50%", background: "var(--vermiglio)", display: "inline-block", animation: "mdPulse 0.8s ease-in-out infinite" }} />
            <span style={{ fontFamily: "var(--mono)", fontSize: 14, color: "var(--ink-2)", letterSpacing: "0.06em" }}>{fmt(recordSec)}</span>
            <span style={{ fontSize: 12, color: "var(--ink-4)", fontFamily: "var(--sans)" }}>Parla con Sofia… rilascia per inviare</span>
          </div>
        ) : (
          <textarea
            ...
            rows={1}
            style={{ flex: 1, resize: "none", border: "none", outline: "none", background: "transparent", fontSize: 13.5, fontFamily: "var(--sans)", color: "var(--ink-1)", lineHeight: 1.5, padding: "6px 4px", maxHeight: 120 }}
          />
        )}

        {text.trim() ? (
          <button onClick={send} title="Invia"
            style={{ width: 34, height: 34, borderRadius: "50%", background: "var(--vermiglio)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "white", flexShrink: 0, transition: "all 0.15s ease" }}
          >
            <Send size={13} />
          </button>
        ) : (
          <button
            ...
            style={{ width: 34, height: 34, borderRadius: "50%", background: recording ? "var(--vermiglio)" : "var(--paper-2)", border: `2px solid ${recording ? "var(--vermiglio)" : "transparent"}`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: recording ? "white" : "var(--ink-3)", flexShrink: 0, transition: "all 0.15s ease", boxShadow: recording ? "0 0 0 4px rgba(201,57,36,0.15)" : "none" }}
          >
      ...
      <div style={{ textAlign: "center", marginTop: 5, fontFamily: "var(--mono)", fontSize: 8.5, color: "var(--ink-5)", letterSpacing: "0.1em" }}>
