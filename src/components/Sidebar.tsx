...
const sb = {
  ...
  text:       "text-[#1a1a1a]",
  muted:      "text-[#7A766F]",
  hover:      "hover:bg-[#E5E0D8]",
  active:     "bg-[#E0D8CE]",
  divider:    "bg-[#D8D3CC]",
  popup:      "bg-[#FAFAF8] border border-[#D8D3CC]",
...
export default function Sidebar({ onOpenModal, isOpen, onToggle, user, onLogout, isPro, onOpenDocumento }: SidebarProps) {
  ...
  return (
    <>
      <div className={`fixed top-0 left-0 h-screen z-[90] flex flex-col ${sb.bg} border-r ${sb.border} transition-all duration-[250ms] ease-in-out overflow-hidden ${isOpen ? "w-[240px]" : "w-0"}`}>

        {/* Logo */}
        <div className={`px-[18px] pt-[18px] pb-[14px] border-b ${sb.border} shrink-0 min-w-[240px] flex items-start justify-between`}>
          <div>
            <div className="font-serif text-[21px] tracking-[-0.5px] text-[#1a1a1a]">
              Norma<span className="text-accent">AI</span>
            </div>
            <div className={`text-[10.5px] ${sb.muted} mt-[2px] italic`}>La norma è uguale per tutti.</div>
          </div>
          <button onClick={onToggle} className={`lg:hidden mt-1 w-7 h-7 flex items-center justify-center ${sb.muted} rounded-md ${sb.hover} transition-all shrink-0`} aria-label="Chiudi menu">
        ...
        <div className={`border-t ${sb.border} shrink-0 min-w-[240px]`}>
          {user ? (
            <>
              <NavItem onClick={() => navClick(() => onOpenModal("bug"))} icon={bugIcon} label="Segnala un bug" />
            ...
            <div className={`px-[18px] py-[14px] text-[10px] ${sb.muted}`}>Servizi Digitali 24 S.R.L.</div>
          )}
        </div>
      </div>
...
function SectionLabel({ label }: { label: string }) {
  return (
    <div className={`px-[18px] pt-[14px] pb-[4px] text-[9.5px] uppercase tracking-[0.09em] font-semibold ${sb.muted}`}>
      {label}
    </div>
  );
}

function Divider() { return <div className={`h-px ${sb.divider} mx-[14px] my-[5px]`} />; }
...
function ToolsMenu({ toggles, onToggle }: { toggles: Record<string, boolean>; onToggle: (id: string) => void }) {
  ...
  return (
    <div ref={ref} className="relative min-w-[240px]">
      {open && (
        <div className={`absolute left-[14px] right-[14px] bottom-full mb-[4px] ${sb.popup} rounded-xl shadow-lg overflow-hidden z-50 max-h-[350px] overflow-y-auto`}>
          {Object.entries(sections).map(([section, items], si) => (
            <div key={section}>
              {si > 0 && <div className={`h-px ${sb.divider} mx-3`} />}
              <div className={`px-4 pt-[10px] pb-[3px] text-[9px] uppercase tracking-[0.08em] ${sb.muted} font-semibold`}>{section}</div>
              {items.map(item => (
                <div key={item.id} className={`flex items-center gap-[9px] px-4 py-[7px] ${sb.popupHover} [&_svg]:w-[13px] [&_svg]:h-[13px] [&_svg]:shrink-0 [&_svg]:stroke-[#7A766F] [&_svg]:fill-none [&_svg]:stroke-[2]`}>
                  {item.icon}
                  <span className={`flex-1 text-[12px] ${sb.text} truncate`}>{item.label}</span>
                  <button onClick={() => onToggle(item.id)} className={`relative w-[28px] h-[15px] rounded-full border-none cursor-pointer transition-colors duration-200 shrink-0 ${toggles[item.id] ? "bg-accent" : "bg-[#C8C3BC]"}`}>
      ...
      <button onClick={() => setOpen(!open)} className={`flex items-center gap-[9px] px-[18px] py-[8px] text-[12px] ${sb.muted} bg-transparent border-none w-full text-left cursor-pointer ${sb.hover} transition-colors duration-150`}>
...
function UserFooter({ userName, userEmail, roleLabel, onLogout, onOpenModal }: { userName: string; userEmail: string; roleLabel: string | null; onLogout: () => void; onOpenModal: (id: string) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      {open && (
        <div className={`absolute bottom-full left-0 w-full mb-[2px] ${sb.popup} rounded-xl shadow-lg overflow-hidden z-50`}>
          <div className={`px-4 pt-3 pb-2 text-[11px] ${sb.muted} truncate border-b ${sb.border}`}>{userEmail}</div>
          <MenuBtn onClick={() => { setOpen(false); onOpenModal("profilo-ai"); }} icon={<svg viewBox="0 0 24 24"><path d="M12 5a3 3 0 00-5.997.125 4 4 0 00-2.526 5.77 4 4 0 00.556 6.588A4 4 0 1012 18z" fill="none" stroke="currentColor" strokeWidth="2"/><path d="M12 5a3 3 0 015.997.125 4 4 0 012.526 5.77 4 4 0 01-.556 6.588A4 4 0 1112 18z" fill="none" stroke="currentColor" strokeWidth="2"/></svg>} label="Il mio profilo AI" sub="Personalizzazione" />
          <MenuBtn onClick={() => { setOpen(false); onOpenModal(roleLabel === "Professionista" ? "professionista" : roleLabel === "Impresa" ? "impresa" : "cittadino"); }} icon={<svg viewBox="0 0 24 24"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z" /><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z" /></svg>} label="Il mio piano" sub={roleLabel === "Cittadino" ? "Gratis" : roleLabel === "Impresa" ? "29€/mese" : roleLabel === "Professionista" ? "29€/mese" : undefined} />
          <div className={`h-px ${sb.divider} mx-2`} />
      ...
      <button onClick={() => setOpen(!open)} className={`w-full flex items-center gap-[10px] px-[14px] py-[12px] bg-transparent border-none text-left cursor-pointer ${sb.hover} transition-colors duration-150`}>
        <div className="w-8 h-8 rounded-full bg-accent/15 border border-accent/30 flex items-center justify-center text-[12px] font-semibold text-accent uppercase shrink-0">{userName.charAt(0)}</div>
        <div className="flex-1 min-w-0">
          <div className={`text-[13px] ${sb.text} font-medium truncate`}>{userName}</div>
          <div className={`text-[10px] ${sb.muted}`}>{roleLabel || "Utente"}</div>
        </div>
        <svg viewBox="0 0 24 24" className={`w-3 h-3 stroke-[#7A766F] fill-none stroke-[2] shrink-0 transition-transform duration-150 ${open ? "rotate-180" : ""}`}><polyline points="6,9 12,15 18,9" /></svg>
      </button>
    </div>
  );
}
...
function MenuBtn({ icon, label, sub, onClick }: { icon: React.ReactNode; label: string; sub?: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-[9px] px-4 py-[9px] text-[13px] ${sb.text} bg-transparent border-none w-full text-left cursor-pointer ${sb.popupHover} transition-colors duration-150 [&_svg]:w-[14px] [&_svg]:h-[14px] [&_svg]:shrink-0 [&_svg]:stroke-current [&_svg]:fill-none [&_svg]:stroke-[2]`}>
      {icon}<span className="flex-1">{label}</span>{sub && <span className={`text-[10px] ${sb.muted}`}>{sub}</span>}
    </button>
  );
}
...
function NavLink({ icon, label, href, onClick }: { icon: React.ReactNode; label: string; href: string; onClick?: () => void }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`flex items-center gap-[9px] px-[18px] py-[8px] text-[13px] ${sb.muted} no-underline ${sb.hover} transition-colors duration-150 [&_svg]:w-[14px] [&_svg]:h-[14px] [&_svg]:shrink-0 [&_svg]:stroke-current [&_svg]:fill-none [&_svg]:stroke-[2]`}
    >
      {icon}{label}
    </Link>
  );
...
function NavItem({ icon, label, onClick, onAdd }: { icon: React.ReactNode; label: string; onClick: () => void; onAdd?: () => void }) {
  return (
    <div className="group flex items-center w-full">
      <button onClick={onClick} className={`flex-1 flex items-center gap-[9px] px-[18px] py-[8px] text-[13px] ${sb.muted} bg-transparent border-none text-left transition-colors duration-150 whitespace-nowrap ${sb.hover} [&_svg]:w-[14px] [&_svg]:h-[14px] [&_svg]:shrink-0 [&_svg]:stroke-current [&_svg]:fill-none [&_svg]:stroke-[2] hover:${sb.text}`}>
        {icon}{label}
      </button>
      {onAdd && (
        <button onClick={(e) => { e.stopPropagation(); onAdd(); }} className={`opacity-0 group-hover:opacity-100 mr-[14px] w-[20px] h-[20px] flex items-center justify-center rounded-md bg-transparent border ${sb.border} ${sb.muted} hover:text-accent hover:border-accent/50 transition-all duration-150 cursor-pointer shrink-0`}>
