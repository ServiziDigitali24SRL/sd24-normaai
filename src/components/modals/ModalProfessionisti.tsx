            ...
            <div className="mt-5 flex flex-col gap-3">
              {selected.telefono && (
                <a href={`tel:${selected.telefono}`}
                  className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[#1e1e1e] hover:border-[#2a2a2a] transition-colors no-underline">
                  ...
                  <span className="text-[13px] text-[#888]">{selected.telefono}</span>
                </a>
              )}
              {selected.email && (
                <a href={`mailto:${selected.email}`}
                  ...
                  <span className="text-[13px] text-[#888]">{selected.email}</span>
                </a>
              )}
              {selected.pec && (
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[#1e1e1e]">
                  <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0 stroke-[#555] fill-none stroke-[1.8]">
                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,12 2,6"/>
                  </svg>
                  <div>
                    <div className="text-[10px] text-[#444] mb-[1px]">PEC</div>
                    <div className="text-[13px] text-[#888]">{selected.pec}</div>
                  </div>
                </div>
              )}
              {selected.note && (
                <div className="px-4 py-3 rounded-xl border border-[#1e1e1e]">
                  <div className="text-[10px] text-[#444] mb-1">Note</div>
                  <div className="text-[12px] text-[#666] leading-[1.5]">{selected.note}</div>
                </div>
              )}
            </div>

            <button onClick={() => handleDelete(selected.id)}
              className="mt-6 text-[11px] text-[#444] hover:text-red-500 transition-colors bg-transparent border-none cursor-pointer p-0">
              Elimina contatto
            </button>
          </div>
        )}
          ...
          <div>
            ...
            <div className="mt-5 flex flex-col gap-3">
                ...
                {CATEGORIE.map(cat => (
                  <button key={cat.id} onClick={() => setForm(f => ({ ...f, categoria: cat.id }))}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-left text-[12px] cursor-pointer transition-all ${
                      form.categoria === cat.id ? "border-accent/40 bg-accent/10 text-cream" : "border-[#1e1e1e] text-[#666] hover:border-[#2a2a2a] bg-transparent"
                    }`}>
                    <span>{cat.icon}</span>
              ...
              {[
                ...
                <div key={key}>
                  <div className="text-[10px] uppercase tracking-[0.06em] text-[#444] mb-[5px]">{label}</div>
                  <input
                    type={type}
                    value={form[key as keyof typeof form]}
                    onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                    className="w-full bg-[#111] border border-[#222] rounded-lg px-3 py-2 text-[13px] text-[#ccc] outline-none focus:border-[#333] placeholder:text-[#333]"
                  />
                </div>
              ))}

              <div>
                <div className="text-[10px] uppercase tracking-[0.06em] text-[#444] mb-[5px]">Note</div>
                <textarea
                  value={form.note}
                  onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
                  rows={2}
                  className="w-full bg-[#111] border border-[#222] rounded-lg px-3 py-2 text-[13px] text-[#ccc] outline-none focus:border-[#333] resize-none"
                />
              </div>
          ...
          <>
            ...
            {pros.length === 0 ? (
              <div className="text-center py-10">
                <div className="text-[13px] text-[#444]">Nessun professionista salvato</div>
                ...
                return (
                  <div key={cat.id} className="mb-5">
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-[9px] uppercase tracking-[0.08em] text-[#444] font-medium">
                        {cat.icon} {cat.label}
                      </div>
                      <button onClick={() => setView("add")}
                        className="text-[10px] text-[#444] hover:text-accent transition-colors bg-transparent border-none cursor-pointer p-0">
                        + aggiungi
                      </button>
                    </div>
                    <div className="flex flex-col gap-[5px]">
                      {items.map(pro => (
                        <button key={pro.id} onClick={() => { setSelected(pro); setView("detail"); }}
                          className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[#1e1e1e] hover:border-[#2a2a2a] hover:bg-white/[0.02] text-left cursor-pointer transition-all w-full bg-transparent">
                          <div className="w-8 h-8 rounded-full bg-[#1a1a1a] border border-[#222] flex items-center justify-center text-[11px] font-semibold text-[#666] shrink-0">
                            {pro.nome[0]}{pro.cognome[0]}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-[13px] font-medium text-[#888] truncate">{pro.nome} {pro.cognome}</div>
                            <div className="text-[11px] text-[#444] truncate">{pro.email || pro.pec || pro.telefono || "—"}</div>
                          </div>
