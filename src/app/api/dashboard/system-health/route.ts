  ...
  const supabase =
    supabaseResult.status === "fulfilled"
      ...
      : {
          ok: false,
          latency: null,
          error: (supabaseResult.reason as Error)?.message,
        };
  ...
  const openRouter =
    openRouterResult.status === "fulfilled"
      ...
      : {
          ...
          limit: null,
          usage: null,
          error: (openRouterResult.reason as Error)?.message,
        };
...
async function fetchOpenRouterCredits(): Promise<{
  ...
  if (!apiKey) {
    return { ok: false, credits: null, limit: null, usage: null, error: "Missing OPENROUTER_API_KEY" };
  }
  ...
  try {
    ...
    const credits =
      limit !== null && usage !== null ? Math.max(0, limit - usage) : null;
